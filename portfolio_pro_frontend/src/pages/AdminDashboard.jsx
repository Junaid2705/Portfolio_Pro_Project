...
import {
  getAdminStats, getAllUsers, toggleUser, deleteUser,
  getAllPortfolios, approvePortfolio, rejectPortfolio
} from '../api/adminApi'
...
export default function AdminDashboard() {
  ...
  const loadAll = async () => {
    ...
    try {
      const [s, u, p] = await Promise.all([
        getAdminStats(), getAllUsers(), getAllPortfolios()
      ])
      setStats(s)
      setUsers(u)
      setPortfolios(p)
  ...
  const handleDeleteUser = async (id, name) => {
    if (!window.confirm(`Delete user "${name}"? This will also delete their portfolio.`)) return
    try {
      await deleteUser(id)
            ...
            {tab === 'Dashboard' && stats && (
              <div className="adm-dashboard">
                ...
                <div className="adm-stats">
                  {[
                    { label: 'Total Users',      value: stats.totalUsers,          icon: '👥', color: '#8b6fff' },
                    { label: 'Total Portfolios', value: stats.totalPortfolios,     icon: '📁', color: '#4f9eff' },
                    { label: 'Published',        value: stats.publishedPortfolios, icon: '🌐', color: '#00c896' },
                    { label: 'Messages',         value: stats.totalMessages,       icon: '✉',  color: '#ffb400' },
                  ].map((s, i) => (
                ...
                <div className="adm-overview-grid">
                    ...
                    <div className="adm-mini-list">
                      {users.slice(0, 5).map(u => (
                        <div key={u.id} className="adm-mini-row">
                          ...
                          <span className={`adm-role-tag ${u.role.toLowerCase()}`}>
                            {u.role}
                          </span>
                        </div>
                      ))}
                    </div>
                    <button className="adm-view-all" onClick={() => setTab('Users')}>
                      View All Users →
                    </button>
                  </div>
                  ...
                  <div className="adm-overview-card">
                    ...
                    <div className="adm-status-breakdown">
                      {[
                        { label: 'Draft',     color: '#6b6988', count: portfolios.filter(p => p.status === 'DRAFT').length },
                        { label: 'Published', color: '#00c896', count: portfolios.filter(p => p.status === 'PUBLISHED').length },
                        { label: 'Rejected',  color: '#ff7070', count: portfolios.filter(p => p.status === 'REJECTED').length },
                      ].map(s => (
                        <div key={s.label} className="adm-status-row">
                          ...
                          <span className="adm-status-count" style={{ color: s.color }}>
                            {s.count}
                          </span>
                    ...
                    <button className="adm-view-all" onClick={() => setTab('Portfolios')}>
                      Manage Portfolios →
                    </button>
                  </div>
                </div>
                  ...
                  <tbody>
                    {filteredUsers.map(user => (
                      <tr key={user.id}>
                        ...
                        <td>
                          <span className={`adm-role-tag ${user.role.toLowerCase()}`}>
                            {user.role}
                          </span>
                        </td>
                        <td>
                          <span className={`adm-status-tag ${user.status.toLowerCase()}`}>
                            {user.status}
                          </span>
                        </td>
                  ...
                  <tbody>
                    {filteredPortfolios.map(p => (
                      <tr key={p.id}>
                        ...
                        <td>
                          <span className={`adm-status-tag ${p.status?.toLowerCase()}`}>
                            {p.status}
                          </span>
                        </td>
                        ...
                        <td>
                          <div className="adm-table-actions">
                            {p.status !== 'PUBLISHED' && (
                              <button className="adm-action-btn activate"
                                onClick={() => handleApprove(p.id)}>
                                Approve
                              </button>
                            )}
                            {p.status === 'PUBLISHED' && (
                              <button className="adm-action-btn deactivate"
                                onClick={() => handleReject(p.id)}>
                                Unpublish
                              </button>
                            )}
                            {p.publicUrlSlug && p.status === 'PUBLISHED' && (
                              <a href={`/p/${p.publicUrlSlug}`} target="_blank"
                                rel="noreferrer" className="adm-action-btn view">
                                View ↗
                              </a>
                            )}
                          </div>
